...
for CNN1_filters in [40,100,256]:#40, ##Move empty variables below for loop
    ...
    #     FCNNOp2=TimeDistributed(MaxPooling1D(3))(FCNNOp1)
        ...
        f_lstm_1=Bidirectional(LSTM(NoUnits, return_sequences=True,activation='tanh'))(CNNOp)
        f_lstm_2a=Bidirectional(LSTM(NoUnits, return_sequences=True,activation='tanh'))(f_lstm_1)
        f_lstm_2=Bidirectional(LSTM(NoUnits, return_sequences=True,activation='tanh'))(f_lstm_2a)
        f_output=TimeDistributed(Dense(12, activation='linear'))(f_lstm_2)
        model = keras.models.Model(mdninput_Lstm,f_output)
        model.summary()
        ...
        model.load_weights('E2E_Results_Batch/JointALsub_Batch5__F1_'+str(CNN1_filters)+'_len160_LSTMunits_150__weights.h5',by_name=True,skip_mismatch=True)
        ...
        OutFileName='FineTue_fromJoint'+ str(BatchSize)+'_'+Sub+'_F1_'+str(CNN1_filters)+'_len'+str(CNN1_flength)+'_LSTMunits_'+str(NoUnits)+'_'
        fName=OutFileName
        print('..fitting model')
